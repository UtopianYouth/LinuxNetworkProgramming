...
int main(int argc, char* argv[]) {
    ...
    inet_pton(AF_INET, "239.0.0.8", &multi_addr.s_addr);
    setsockopt(communication_fd, IPPROTO_IP, IP_MULTICAST_IF, &multi_addr, sizeof(multi_addr));

    // 3. 设置多播的 IP + port
    struct sockaddr_in client_addr;
    client_addr.sin_family = AF_INET;
    inet_pton(AF_INET, "239.0.0.8", &client_addr.sin_addr.s_addr);  // 多播地址
